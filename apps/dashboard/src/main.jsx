...
function Overview({liveTick}){
 ...
 return <div className="page"><div className="title"><div><h1>Overview</h1><p>Production health across connected projects.</p></div><button onClick={()=>window.open("https://github.com/new","_blank")}><Plus size={15}/> Add project</button></div>
...
function Projects({open,liveTick}){
 ...
 return <div className="page"><div className="title"><div><h1>Projects</h1><p>Connected repositories and production systems.</p></div>{owner?<button onClick={()=>window.open("https://github.com/new","_blank")}><Plus size={15}/> Connect repository</button>:<Badge tone="blue">Public read-only view</Badge>}</div>
 ...
 <div className="projectGrid">{sorted.map(p=>(<div className="projectCard" key={p.id} onClick={()=>open(p)}>{PG[p.id]?<div className="projectCardBg"><img src={PG[p.id]} alt="" loading="lazy"/></div>:null}<div className="projectCardOverlay"/><div className="projectCardContent"><div className="projectTop"><span className="projectIcon"><Github size={16}/></span><Badge tone={p.status==="Healthy"?"green":"yellow"}>{p.status}</Badge></div><h3>{p.name}</h3><small className="repoPath">{p.repo}</small><div className="metrics"><div><span>Uptime</span><b><CountUp n={p.uptime}/>%</b></div><div><span>Services</span><b>{summary.services[p.id]||0}</b></div><div><span>Incidents</span><b>{summary.incidents[p.id]||0}</b></div></div><div className="projectFoot"><strong>Open project <ChevronRight size={14}/></strong></div></div></div>))}{sorted.length===0?<p className="muted full">No projects match your filters.</p>:null}</div></div>
}
...
function App(){
 ...
 const nav=[["Overview",LayoutDashboard],["Projects",Box],["About",User],["Analytics",BarChart3],["Settings",Settings]];
 let content=selected?<ProjectDetails p={selected} back={()=>setSelected(null)} liveTick={liveTick}/>:page==="Overview"?<Overview liveTick={liveTick}/>:page==="Projects"?<Projects open={setSelected} liveTick={liveTick}/>:page==="About"?<AboutPage/>:page==="Analytics"?<AnalyticsPage/>:<SettingsPage/>;
 return <><div className="liquidBg"/><div className="liquidOrb o1"/><div className="liquidOrb o2"/><div className="liquidOrb o3"/><div className="app"><aside><div className="brand">Pulse<span style={{color:'var(--accent)'}}>Ops</span></div><div className="workspace"><b>Acme Engineering</b><small>Production</small></div><nav>{nav.map(([n,I])=><button className={page===n&&!selected?"active":""} onClick={()=>{setPage(n);setSelected(null)}} key={n}><I size={17}/>{n}</button>)}</nav><div className="user">AS · Ansh Sharma</div></aside> <main><header><span>{selected?.name||page}</span><span className="liveBadge"><i/>LIVE</span><div className="top">{owner?<Badge tone="green">Owner</Badge>:<Badge tone="blue">Public view</Badge>}<div className="topSearch"><Search size={14}/><input placeholder="Search…"/></div><button className="fbBtn" onClick={()=>setFbOpen(true)} title="Leave your name so I know who visited"><MessageSquare size={13}/> Feedback</button><Bell size={17}/><span className="avatar">AS</span></div></header>{content}</main><button className="chatFab" onClick={()=>setChatOpen(true)} title="Ask the AI about any project"><MessageSquare size={17}/> Ask AI</button>{chatOpen?<ChatDrawer onClose={()=>setChatOpen(false)}/>:null}<FeedbackModal open={fbOpen} setOpen={setFbOpen}/></div></>
}
